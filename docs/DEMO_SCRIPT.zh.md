...
强调：客户可见 draft 进入 role-based approval flow；talking points、term explainer、next actions 是 RM 内部准备工具，只保留 trace，不需要经理逐条审批。
...
- Adrian 的 routine service draft，例如 Quick check-in、Appointment confirmation、Phone opener，是 RM 显式 self-approval 后使用，不进入 Sofia 的 `Draft approval queue`。
...
3. 展示 trace 里的 `approvalRequired: rm-approval`，说明 routine draft 是 Adrian 自己显式 approval，不会出现在 Sofia 队列。
...
- 同一入口，不同权限梯度。
- Adrian 不是绕过审批；routine 服务触达仍有 self-approval audit，只是减少经理队列噪音。
...
6. 打开示范历史 draft（Violet Carter），展示 `History`：created -> returned -> edited -> approved -> sent。重点讲退回意见如何被执行：yield projection 被删除，改成 review-meeting invitation。
7. 在 Management 队列或 handled evidence 上点击 `Export evidence`，导出可转发 PDF；再在 Client 360 的 trace 面板点击 `Export this trace`，说明单次 AgentRun 也能导出 trace 版证据。
...
- Manager 负责审批客户可见内容。
- Audit 记录每一步。
- 这份 evidence pack 是买方可转给老板、风控和 IT 的审计重建样本；正文只保留业务可读证据，技术细节放在附录或省略。
